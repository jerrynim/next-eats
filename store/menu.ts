...
//type정의
const TRIGGER_GET_MENU = "menu/TRIGGER_GET_MENU";
...
export function* asyncGetMenu() {
  try {
    ...
    //fetched : {menus: Array(3)}
    yield put<GetMenu>({ type: "menu/GET_MENU", payload: fetched.menus });
  } catch (e) {
...
export function* saga() {
  yield takeEvery("menu/TRIGGER_GET_MENU", asyncGetMenu);
}
...
export const menu = (state = initialState, action: Actions) =>
  produce(state, (draft) => {
    switch (action.type) {
